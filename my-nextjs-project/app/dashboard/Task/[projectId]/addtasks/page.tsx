...
export default function AddTasksPage() {
  ...
  // --- Auth helper ---
  // The frontend will look for a token in localStorage under key 'token'.
  // If you use JWT set localStorage.setItem('authScheme', 'bearer') after login.
  // Default behavior: TokenAuth -> 'Token <token>'. If authScheme === 'bearer' -> 'Bearer <token>'.
  const token = typeof window !== "undefined" ? localStorage.getItem("token") : null;
  const authScheme = typeof window !== "undefined" ? localStorage.getItem("authScheme") : null; // 'bearer' for JWT
  ...
  const saveEdit = async () => {
    ...
    const { id, sequence_id, project, created_at, updated_at, ...body } = editingTask as any;
    setError(null);
    try {
  ...
  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div className="bg-white rounded p-4 w-[420px]">
      ...
      <div className="d-flex justify-content-end gap-2">
        <button className="g-btn" onClick={() => setShowAddModal(false)}>
          Close
        </button>
        <button
          className="g-btn"
...
- After a successful login, store the token in localStorage:
  ...
  // if using JWT, also set the scheme:
  localStorage.setItem('authScheme', 'bearer');

- If you rely on session authentication, ensure the login endpoint sets a session cookie and the browser includes it. The component keeps credentials: 'include' and sends the CSRF token from the csrftoken cookie.
*/
