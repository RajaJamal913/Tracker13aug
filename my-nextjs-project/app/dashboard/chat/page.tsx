...
export default function ChatPage(): JSX.Element {
  ...
  // ---------- UI ----------
  return (
    <div className="flex h-screen bg-gray-50 chat-main-wrapper">
      {/* Sidebar */}
      <aside className="w-80 border-end text-gray-800 flex flex-col" style={{background: "#e6f0ff;"}}>
        <div className="p-4 border-bottom border-purple-500">
          ...
          <div className="mt-3 relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-grey/70/70" size={16} />
            <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-3 py-2 rounded bg-white bg-opacity-10 placeholder-white/60 outline-none border-1" placeholder="Search members..." />
          </div>
        </div>

        <div className="flex-1 overflow-auto">
          {/* Channels header */}
          <div className="p-3 border-bottom border-purple-500 flex items-center justify-between">
            <span className="font-semibold">Channels</span>
            <div className="flex items-center gap-2">
              <button onClick={() => { fetchChannels() }} className="text-grey/70/80">⟳</button>
          ...
          <div className="p-3 space-y-1 max-h-[30vh] overflow-auto">
            {channels.length === 0 ? (
              <div className="text-grey/70">No channels</div>
            ) : channels.map(c => (
              <div key={c.id} className="flex items-center justify-between p-2 rounded hover:bg-white/5">
                ...
                <div className="flex items-center gap-2">
                  {channelUnread[String(c.id)] ? <span className="bg-red-500 text-grey/70 text-xs rounded-full px-2">{channelUnread[String(c.id)]}</span> : null}
          ...
          <div className="p-3 border-bottom border-purple-500 flex items-center justify-between">
            ...
            <button className="ml-2 text-grey/70/80" onClick={() => { fetchMembers(); fetchConversations() }}>Refresh</button>
          </div>

          <div className="p-3 space-y-1 max-h-[60vh] overflow-auto">
            {isLoadingMembers ? (
              <div className="text-grey/70/80">Loading members...</div>
            ) : filteredMembers.length === 0 ? (
              <div className="text-grey/70/70">No members found</div>
              ...
              <div key={m.id} className="flex items-center justify-between p-2 rounded hover:bg-white/5">
                ...
                <div className="flex items-center gap-2">
                  {memberUnread[String(m.id)] ? <span className="bg-red-500 text-grey/70 text-xs rounded-full px-2">{memberUnread[String(m.id)]}</span> : null}
      ...
      <main className="flex-1 flex flex-col">
        <div className="border-bottom bg-white px-3 py-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mb-0">{activeConversationType === "dm" ? (activeRecipientId ? `DM with ${activeRecipientName || activeRecipientId}` : "") : (activeChannelId ? `Channel ${activeChannelId}` : "")}</p>
          </div>
          <div className="flex items-center space-x-2">
          ...
          {messages.map(m => {
            ...
            return (
              <div key={String(m.id)} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-md p-3 rounded-lg ${isOwn ? "bg-purple-100 text-black" : "bg-gray-800 text-grey/70"}`}>
        ...
        <div className="bg-white border-t p-3">
          <form onSubmit={(e) => {
            ...
            <button type="submit" disabled={!newMessage.trim() && (uploading || !(activeConversationType === "dm" ? activeRecipientId : activeChannelId))} className="px-4 py-2 rounded bg-primary text-white" style={{minHeight:"41px"}}><Send size={16} /></button>
          </form>
        </div>
      </main>

      {/* Right column */}
      <aside className="w-64 border-start hidden  bg-white">
        <div className="p-3 border-bottom">Online Now</div>
      ...
      {showCreateChannel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded p-4 shadow-lg" style={{background:"aliceblue"}}>
            ...
            <label className="block">
              ...
              <input value={newChannelName} onChange={(e) => setNewChannelName(e.target.value)} className="w-100 mt-1 p-2 border rounded" placeholder="Team chat, Project A..." />
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowCreateChannel(false)} className="g-btn">Cancel</button>
              <button onClick={() => createChannel()} className="g-btn">Create</button>
            </div>
          </div>
        </div>
      )}
